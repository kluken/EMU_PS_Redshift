...
def main():
    parser = argparse.ArgumentParser(
        description="This script runs the GPz algorithm, after reading, splitting and creating datasets"
    )
    parser.add_argument(
        "-s",
        "--seed",
    ...
    combined_data[np.where(combined_data[:, -4] == -99), -4] = np.nanmax(
        combined_data[:, -4]
    )
    combined_data[np.where(combined_data[:, -3] == -99), -3] = np.nanmax(
        combined_data[:, -3]
    )
    combined_data[np.where(combined_data[:, -2] == -99), -2] = np.nanmax(
        combined_data[:, -2]
    )
    combined_data[np.where(combined_data[:, -1] == -99), -1] = np.max(
        combined_data[:, -1]
    )

    combined_data[np.where(np.isnan(combined_data[:, -4])), -4] = np.nanmax(
        combined_data[:, -4]
    )
    combined_data[np.where(np.isnan(combined_data[:, -3])), -3] = np.nanmax(
        combined_data[:, -3]
    )
    combined_data[np.where(np.isnan(combined_data[:, -2])), -2] = np.nanmax(
        combined_data[:, -2]
    )
    combined_data[np.where(np.isnan(combined_data[:, -1])), -1] = np.nanmax(
        combined_data[:, -1]
    )
    ...
    x_vals_train, x_vals_test, y_vals_train, y_vals_test = split_data(
        np.copy(x_vals), np.copy(y_vals), seed, test_split_rate
    )

    # for gmm_comp in gmm_components_list:
    gmm_model = GaussianMixture(n_components=gmm_comp, max_iter=500).fit(x_vals_train)
    ...
    for component in np.arange(gmm_model.n_components):
        ...
        if not os.path.isfile(test_file_name):
            ...
            train_table.write(
                train_file, format="ascii.commented_header", overwrite=True
            )
    ...
    pred_table_stack = Table.read(
        pred_file_name, format="ascii.commented_header", header_start=10
    )
    ...
    test_table_stack.write(
        "predictions_seed_" + str(seed) + "_comp_" + str(gmm_comp) + ".csv",
        format="csv",
        overwrite=True,
    )
    train_table_stack.write(
        "train_seed_" + str(seed) + "_comp_" + str(gmm_comp) + ".csv",
        format="csv",
        overwrite=True,
    )

    hex_plot_filename = (
        "final_plot_seed_" + str(seed) + "_comp_" + str(gmm_comp) + ".pdf"
    )
    plot_hex(
        test_table_stack["z_spec"], test_table_stack["prediction"], hex_plot_filename
    )
    out_rate = outlier_rate(
        norm_residual(test_table_stack["z_spec"], test_table_stack["prediction"])
    )
